...
class FNO2d(nn.Module):
    def __init__(self):
        ...
        self.decode = self.q = MLP(self.latent_dims, 1, self.latent_dims * 4)
...
if __name__ == '__main__':
    ...
    data_module = PDEDataModule({
        'DATA_FILE': 'ns_V1e-3_N5000_T50.mat',
    ...
    train_loader, val_loader, test_loader, _, _ = data_module.get_data_loaders()

    # reader = MatReader(TRAIN_PATH)
    # train_a = reader.read_field('u')[:ntrain,::sub,::sub,:T_in]
    # train_u = reader.read_field('u')[:ntrain,::sub,::sub,T_in:T+T_in]

    # val_a = reader.read_field('u')[ntrain:ntrain+nval,::sub,::sub,:T_in]
    # val_u = reader.read_field('u')[ntrain:ntrain+nval,::sub,::sub,T_in:T+T_in]

    # test_a = reader.read_field('u')[ntrain+nval:ntrain+nval+ntest,::sub,::sub,:T_in]
    # test_u = reader.read_field('u')[ntrain+nval:ntrain+nval+ntest,::sub,::sub,T_in:T+T_in]
    ...
    # trainer
    ################################################################
    from trainer_module.TrainerModule import TrainerModule
    ...
    class LightningModule(pl.LightningModule):
        ...
        def training_step(self, batch, batch_idx):
            loss, pred = self.run_batch(batch)
    ...
    trainer.predict(model=lightning_module, dataloaders=test_loader)