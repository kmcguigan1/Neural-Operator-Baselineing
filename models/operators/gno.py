...
class KernelNN(torch.nn.Module):
    def __init__(self, config:dict): #width, ker_width, depth, ker_in, in_width=1, out_width=1):
        super().__init__()
        # save needed vars
        ...
        self.projector = torch.nn.Linear(self.in_dims, self.latent_dims)
        kernel = DenseNet([self.edge_dims, self.kernel_dims, self.kernel_dims, self.latent_dims**2], torch.nn.ReLU)
        self.conv = NNConv_old(self.latent_dims, self.latent_dims, kernel, aggr='mean')
        self.decoder = torch.nn.Linear(self.latent_dims, 1)
