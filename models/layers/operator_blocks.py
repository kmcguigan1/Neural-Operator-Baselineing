...
from models.layers.operator_layers import SpectralConv2d, DenseNet, NNConv
from models.layers.base_layers import ConvMLP

class FNOBlock(nn.Module):
    def __init__(self, latent_dims, modes1, modes2, activation=nn.Identity):
        super().__init__()
        # save the variables
...
class GNOBlockSingleConv(nn.Module):
    def __init__(self, latent_dims:int, kernel_dims:int, edge_dims:int, depth:int, activation:function=F.gelu, activation_first:bool=False):
        # save some variables
        self.latent_dims = latent_dims
