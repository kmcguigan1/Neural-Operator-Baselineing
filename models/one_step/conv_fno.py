...
from models.layers.fourier_blocks import FNOBlock

class ConvFNOBranch(nn.Module):
    def __init__(self, kernel_size, latent_dims, modes1, modes2, mlp_ratio, padding_mode):
        super().__init__()
        self.conv = FNOBlock(latent_dims, modes1, modes2, mlp_ratio, padding_mode=padding_mode)
        self.w = nn.Conv2d(latent_dims, latent_dims, kernel_size, padding=kernel_size//2)
...
class ConvFNO(nn.Module):
    def __init__(self, config:dict):
        ...
        for idx in range(self.depth):
            self.blocks.append(ConvFNOBranch(self.kernel_size, self.latent_dims, self.modes[0], self.modes[1], self.mlp_ratio, padding_mode=self.padding_mode))
            if(idx < self.depth - 1):
