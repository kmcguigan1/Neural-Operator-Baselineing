...
class GFNOBlock(nn.Module):
    def __init__(self, latent_dims, modes1, modes2, edge_dims:int, mlp_ratio:int=None):
        super().__init__()
        # vars
        ...
        self.gno_block = GNOBlockEfficient(self.latent_dims, self.edge_dims, mlp_ratio=mlp_ratio)
...
class GFNOEfficient(nn.Module):
    def __init__(self, config:dict):
        ...
        self.blocks = nn.ModuleList([GFNOBlock(self.latent_dims, self.modes[0], self.modes[1], self.edge_dims) for _ in range(self.depth)])   
