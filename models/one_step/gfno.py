...
from models.layers.fourier_blocks import TokenFNOBranch
from models.layers.graph_blocks import GNOBlockSingleConv, GNOBlockSingleConvAddInitNodesToEdge, GNOBlockSingleConvAddNodesToEdge
...
class GFNOBlock(nn.Module):
    def __init__(self, latent_dims, modes1, modes2, kernel_dims:int, edge_dims:int, graph_passes:int, add_init_to_edge:bool, add_node_to_edge:bool, padding_mode:str=None):
        ...
        if(self.add_init_to_edge):
            self.edge_dims += 2
            self.gno_block = GNOBlockSingleConvAddInitNodesToEdge(self.latent_dims, self.latent_dims, self.kernel_dims, self.edge_dims, self.graph_passes)
...
class GFNO(nn.Module):
    ...
    def forward(self, nodes, grid, edge_index, edge_attrs, batch_size, image_size):
        ...
        for block in self.blocks:
            nodes = block(nodes, edge_index, edge_attrs, batch_size, image_size, init=init)
